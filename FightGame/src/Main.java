...
public class Main extends JFrame {
    ...
    private void openGamePage() {
    	 if (selectedLanguage == 0) {
             ...
             GamePage frame = new GamePage(player1Name, player2Name);
             frame.setLocationRelativeTo(null); // set location to the middle.
             frame.setVisible(true);
             dispose();
         } else if (selectedLanguage == 1) {
             // Turkish version (GamePageTR)
             String player1Name = player1.getText().trim();
             String player2Name = player2.getText().trim();
             GamePageTR frame = new GamePageTR(player1Name, player2Name); // Assuming the constructor is available
    ...
    public Main() {
        ...
        JLabel lblNewLabel = new JLabel("BUTTON BASED FIGHT GAME");
        ...
        JLabel player1_name = new JLabel("Player 1 :");
        ...
        JLabel player1_name_1 = new JLabel("Player 2 :");
        ...
        errorMessage = new JLabel("ENTER A VALID NAME");
        ...
        lang_btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
                if (selectedLanguage == 0) {
                    selectedLanguage = 1;
                    lang_btn.setText("TR");
                    dispose(); // Close the current frame (Main)
                    // Create a new instance of MainTR and show it
                    EventQueue.invokeLater(new Runnable() {
                        @Override
						public void run() {
                            try {
                                MainPageTR frameTR = new MainPageTR();
                                frameTR.setLocationRelativeTo(null);
                    ...
                    selectedLanguage = 0;
                    lang_btn.setText("EN");
                    dispose(); // Close the current frame (MainTR)
